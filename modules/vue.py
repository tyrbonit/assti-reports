...
class Vue(Storage):
    ...
    def share(cls, key='getvue'):
        """Декоратор с параметрами для выдачи однофайлового компонента Vue
           ...
           со значением, содержащим путь к файлу компонента.
        ...
        class decorator(object):
            ...
            def __call__(self, action):
                ...
                def wrapper():
                    ...
                    if request.env.http_vue_component_file:
                        args = request.env.http_vue_component_file.split('/')
                        if key in args:
                            args = args[args.index(key):]
            ...
            def __repr__(self):
                args = [key]
                return serializers.json({k:'url:%s'%URL(args=args+v.split(os.path.sep)) for k, v in cls.search_components().items()})
